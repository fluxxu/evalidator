    ...
    var execRule_ = function (attr, rule, errorMap, done) {
        ...
        if (_.isObject(rule) && rule.validator) {
            ...
            if (!(validatorName in Validator.prototype)) {
                done(new Error('Validator not found: ' + validatorName));
                return;
            }
            try {
                var func = Validator.prototype[validatorName];
            ...
            if (rule.length == 2) {
                ...
                //async
                //value, context, done
                rule.call(rule, value, ctx, done);
            } else {
                done(new Error("Invalid validate function for '" + attr + "': " + rule));
            }
        } else {
            done(new Error("Invalid validate rule for '" + attr + "': " + rule));
        }
    };
